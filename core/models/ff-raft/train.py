...
from common import yaml_parser, setup, parallel_model
from FF_RAFT_Core.ff_raft import FF_RAFT_FUSION
...
class Logger:
    def __init__(self, log_dir, yaml):
        ...
        copytree("./FF_RAFT_Core/", f"{self.log_dir}/core/", dirs_exist_ok=True)
...
def train(args, cfg):
    ...
    data_root["chairs"] = "../../../data/FlyingChairs_release"
    ...
    load_raft = cfg.MODEL.LOAD_MODULE
    if rank in [-1, 0]:
        print(f"Load pre-trained FF-RAFT module: {cfg.MODEL.PRETRAIN}")
        print(f"Load pre-trained RAFT module: {load_raft}")
        print(f"Fusion mode: {cfg.MODEL.FUSION}")
        print(f"Fusion channels: {cfg.MODEL.FUSION_CHANNEL}")
        print(f"Use small RAFT: {cfg.MODEL.SMALL}")
        print(f"Dropout: {cfg.MODEL.DROPOUT}")
        print(f"Use CUDA correlation volume: {cfg.MODEL.ALT_CORR}")
        print(f"Abandon fnet in RAFT: {cfg.MODEL.ABANDON_FNET}")
        print(f"Fuse cnet of RAFT: {cfg.MODEL.FUSE_CNET}")
        print(f"Freeze flow net: {cfg.MODEL.FREEZE_MODULE}")
    
        logger.log_info(f"Model config : {cfg.MODEL}")
    
    model = FF_RAFT_FUSION(pretrain=cfg.MODEL.PRETRAIN, load_raft=load_raft, use_fusion=cfg.MODEL.FUSION, fusion_channels=cfg.MODEL.FUSION_CHANNEL, raft_small=cfg.MODEL.SMALL,
                    dropout=cfg.MODEL.DROPOUT, alternate_corr=cfg.MODEL.ALT_CORR, abandon_fnet=cfg.MODEL.ABANDON_FNET, fuse_cnet=cfg.MODEL.FUSE_CNET,
                    freeze_flownet=cfg.MODEL.FREEZE_MODULE, cfg=cfg)
    if rank in [-1, 0]:
        print("\rModel initialized.")
    
    if cfg.TRAIN.STAGE != 'chairs':
        model.flow_net.freeze_bn()
    ...
    if rank in [-1, 0]:
        print(f"Optimizer: {type(optimizer)}---Lr: {optimizer.state_dict()['param_groups'][0]['lr']}")
        logger.log_info(f"Optimizer: {type(optimizer)}---Lr: {optimizer.state_dict()['param_groups'][0]['lr']}")
    
    if cfg.TRAIN.SEPARATE_LR:
        optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.fusion_layer.parameters()),
                            lr=cfg.OPTIMIZER.LR,
                            weight_decay=cfg.OPTIMIZER.WEIGHT_DECAY,
                            eps=cfg.OPTIMIZER.EPSILON)
        optimizer1 = optim.AdamW(filter(lambda p: p.requires_grad, model.flow_net.parameters()),
                            lr=cfg.OPTIMIZER.LR/10.,
                            weight_decay=cfg.OPTIMIZER.WEIGHT_DECAY,
                            eps=cfg.OPTIMIZER.EPSILON)
        
        if rank in [-1, 0]:
            print(f"Optimizer1: {type(optimizer1)}---Lr: {optimizer1.state_dict()['param_groups'][0]['lr']}")
            logger.log_info(f"Optimizer1: {type(optimizer1)}---Lr: {optimizer1.state_dict()['param_groups'][0]['lr']}")
            print("Using separate learning rate.\n")
    ...
    scheduler = None
    if cfg.TRAIN.STAGE == "chairs":
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, cfg.SCHEDULER.LR, cfg.TRAIN.NUM_STEPS + 100, pct_start=0.05,
        ...
        raise NotImplementedError(f"Stage {cfg.TRAIN.STAGE} not supported.")
        
    if rank in [-1, 0]:
        print(f"Scheduler: {type(scheduler)}--Lr: {cfg.SCHEDULER.LR}")
        logger.log_info(f"Scheduler: {type(scheduler)}")
        
    if cfg.TRAIN.SEPARATE_LR:
        scheduler1 = optim.lr_scheduler.OneCycleLR(optimizer1, cfg.SCHEDULER.LR/10., cfg.TRAIN.NUM_STEPS + 100, pct_start=0.05,
                                                  cycle_momentum=False, anneal_strategy="linear")
        if rank in [-1, 0]:
            print(f"Scheduler1: {type(scheduler1)}--Lr: {cfg.SCHEDULER.LR/10.}")
            logger.log_info(f"Scheduler1: {type(scheduler1)}")
    ...
    while should_keep_training:
        for step, data_blob in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=cfg.GLOBAL.MIXED_PRECISION):
                flow_predictions = model(image1, image2, mask1, mask2, raft_iters=cfg.MODEL.ITERS)
            ...
            if cfg.TRAIN.SEPARATE_LR:
                ...
                scheduler1.step()
                metrics['lr1'] = optimizer1.state_dict()['param_groups'][0]['lr']

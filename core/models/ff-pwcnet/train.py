...
from PWCNet_Core.ff_pwcnet import FF_PWCNET
...
class Logger:
    def __init__(self, log_dir, yaml):
        ...
        # copyfile("./train.sh", f"{self.log_dir}/train.sh")
        copytree("./PWCNet_Core/", f"{self.log_dir}/core/", dirs_exist_ok=True)
...
def train(args, cfg):
    ...
    load_pwc = cfg.MODEL.LOAD_MODULE
    if rank in [-1, 0]:
        print(f"Pretrain: {pretrain}")
        print(f"Load PWC: {load_pwc}")
    ...
    model = FF_PWCNET(cfg, pretrain=pretrain, load_pwcnet=load_pwc)
    ...
    scheduler = None
    if cfg.TRAIN.STAGE == "chairs":
        # scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[400000, 600000, 800000, 1000000], gamma=0.5)
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, cfg.SCHEDULER.LR, cfg.TRAIN.NUM_STEPS + 100,
                                                  pct_start=0.05,
                                                  cycle_momentum=False, anneal_strategy="linear")
    elif cfg.TRAIN.STAGE == "things":
        # scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[200000, 300000, 400000], gamma=0.5)
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, cfg.SCHEDULER.LR, cfg.TRAIN.NUM_STEPS + 100,
                                                  pct_start=0.05,
                                                  cycle_momentum=False, anneal_strategy="linear")
    ...
    if cfg.TRAIN.SEPARATE_LR:
        scheduler1 = optim.lr_scheduler.OneCycleLR(optimizer1, cfg.SCHEDULER.LR / 10., cfg.TRAIN.NUM_STEPS + 100,
                                                   pct_start=0.05,
                                                   cycle_momentum=False, anneal_strategy="linear")
        if rank in [-1, 0]:
    ...
    loss_function = build_losses(cfg)
    ...
    # a = evaluate.validate_chairs(model, cfg.TRAIN.MASK_TYPE, cfg)
    # print(a)

    sparse = False
    if cfg.TRAIN.STAGE == "kitti":
        sparse = True
    ...
    while should_keep_training:
        for step, data_blob in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=cfg.GLOBAL.MIXED_PRECISION):
                ...
                loss, metrics = loss_function(flow_predictions, flow, mask1, sparse)
            ...
            if rank in [-1, 0]:
                ...
                if total_steps % VAL_FREQ == VAL_FREQ - 1:
                    PATH = f'{LOG_DIR}/checkpoints/{cfg.GLOBAL.NAME}.pth.tar'
                    checkpoint = {
                        'step': total_steps + 1,
            ...
            if current_training_step % 100 == 99:
                ...
                if rank in [-1, 0]:
                    logger.log_info(f"---One step used time: {avg_use_time}", end=" ")

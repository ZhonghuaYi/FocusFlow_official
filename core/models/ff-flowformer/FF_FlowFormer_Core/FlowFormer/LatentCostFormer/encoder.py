...
from ..encoders import twins_svt_large, twins_svt_large_CCE
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x) -> Tuple[torch.Tensor, Size_]:
        ...
        # print(f"after 1x1 conv: {x.shape}")
...
class CostPerceiverEncoder(nn.Module):
    ...
    def forward(self, cost_volume, data, context=None):
        ...
        # print(f"cost_maps.shape: {cost_maps.shape}")
        x, size = self.patch_embed(cost_maps)   # B*H1*W1, size[0]*size[1], C
        data['H3W3'] = size
        H3, W3 = size

        # print(f"x.shape:{x.shape}")
        # print(f"C.shape:{self.latent_tokens.shape}")
...
class MemoryEncoder(nn.Module):
    ...
    def forward(self, img1, img2, data, context=None):
        ...
        B = feats.shape[0] // 2
        self.fmap = feats[:B].detach().clone()
...
class Fusion_MemoryEncoder(nn.Module):
    def __init__(self, cfg):
        super(Fusion_MemoryEncoder, self).__init__()
        self.cfg = cfg

        # if cfg.fnet == 'twins':
        #     self.feat_encoder = twins_svt_large_CCE()
        # elif cfg.fnet == 'basicencoder':
        #     self.feat_encoder = BasicEncoder(output_dim=256, norm_fn='instance')
        # else:
        #     exit()
    ...
    def forward(self, img1, img2, mask1, mask2, data, context=None):
        # The original implementation
        feat_s = self.feat_encoder(img1, mask1)
        feat_t = self.feat_encoder(img2, mask2)
        
        self.fmap = feat_s.detach().clone()

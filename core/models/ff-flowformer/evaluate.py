...
def validate_chairs(model, mask_type):
    ...
    val_loader = data.DataLoader(val_dataset, batch_size=8, pin_memory=True, shuffle=False, num_workers=8, drop_last=False)
    for _, val_data in enumerate(val_loader):
        image1, image2, flow_gt, mask1, mask2, _ = [x.cuda() for x in val_data]

        flow_pr = model(image1, image2, mask1, mask2)
        epe = (flow_pr[0] - flow_gt) ** 2
